...
설계 스펙만으로 전기차 주행거리를 사전 예측하는 선형 회귀 모델 프로젝트입니다.
...
| 항목 | 내용 |
|:---|:---|
| 규모 | 442개 전기차 모델, 22개 변수 |
| 종속변수 (y) | `range_km` — 완전 충전 시 주행 가능 거리 |
| 독립변수 (X) | `battery_capacity_kWh`, `efficiency_wh_per_km`, `top_speed_kmh`, `height_mm`, `drivetrain`, `segment` 등 |
...
**데이터 분할** — Train / Test 8:2 분할 (`random_state=2`)
...
**성능 평가** — 10-Fold Cross Validation 수행했음
